...
import com.baidu.idl.face.main.utils.SerialPortUtils;
...
public class TcpService extends Service {
    ...
    public static final int GPIO = 42;
...
//        LiveDataBus.get().with(BaseConstant.slabShutdownMessage, Integer.class).observeForever(getSlabShutdownMessageObserver);
    ...
    private void exportGpio(int gpio) {
...
//                                    FaceApi.getInstance().initDatabases(true);
                                    //刷新人脸库
                                    FaceSDKManager.getInstance().initDataBases(getApplicationContext());
//                                    FaceSDKManager.getInstance().refreshMemory(getApplicationContext());
...
//                                    mQueue.offer(Utils.addBytes(mFaceLib, infoCode, mFaceLibOk));
    ...
    private void registerStartAddress(byte[] buffer, int size) {
        ...
        if (inTotalInfoBool) {
            ...
            Log.e(TAG, "registerStartAddress总人员信息: " + inTotalInfo);
        }
        //获取后台人脸特征库
        ...
        singleThreadExecutor.execute(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = 1; i <= personnelInfoSize / 536; i++) {
                    ...
                    try {
                        username = new String(nameByte, "GB2312").trim();
...
//                            "  getCard: " + FaceApi.getInstance().getUserByUserName("default", username) + "---->" + !(username.trim().equals(FaceApi.getInstance().getUserByUserName("default", username))));
                    ...
                    //根据卡号判断是否存在
                    if (!(card.equals(FaceApi.getInstance().getUserCardByUserCard(card))) && !Utils.isMessyCode(username)) {
                        //添加到数据库中
                        isSuccess = FaceApi.getInstance().registerUserIntoDBmanager(null, username, "", card, featureByte);
                        if (isSuccess) {
                            success++;
                            Log.e(TAG, "插入 用户 " + username + " 卡号 " + card);
                        }
                    }
