...
public class FileUtils {
     ...
     * @return
     */
    public static boolean isFileExist(Context context, String imgDirectory) {
        String[] sdRootFile = getAllSdPaths(context);
        if (sdRootFile.length >= 2) {
            File file = new File(sdRootFile[1] + File.separator + imgDirectory);
            return file.exists();
        }
        return false;
    }
     ...
     * @return
     */
//    public static List<String> getImagePathFromSD(Context context, String imgDirectory) {
//        //得到所有的存储路径（内部存储+外部存储）
//        //strAllPath[0] 机身储存       strAllPath[1] = 外部储存卡
//        String[] strAllPath = getAllSdPaths(context);
//        // 图片列表
//        List<String> imagePathList = new ArrayList<>();
//        // 得到sd卡内image文件夹的路径   File.separator(/)
//        String filePath = strAllPath[1] + File.separator + imgDirectory;
//        // 得到该路径文件夹下所有的文件
...
//    }
     ...
     * @return
     */
    public static HashMap<String, String> getImagePathFromSD(Context context, String imgDirectory) {
        //得到所有的存储路径（内部存储+外部存储）
        //strAllPath[0] 机身储存       strAllPath[1] = 外部储存卡
        String[] strAllPath = getAllSdPaths(context);
        // 图片列表
        ...
        File fileAll = new File(filePath);
        File[] files = fileAll.listFiles();
        // 将所有的文件存入imagePathMap中,并过滤所有图片格式的文件
        for (File file : files) {
            if (checkIsImageFile(file.getPath())) {
                imagePathMap.put(file.getName(), file.getPath());
            }
        }
        // 返回得到的图片列表
        return imagePathMap;
     ...
     * @return
     */
    public static int getImagePathFromSDLength(Context context, String imgDirectory) {
        //得到所有的存储路径（内部存储+外部存储）
        //strAllPath[0] 机身储存       strAllPath[1] = 外部储存卡
        String[] strAllPath = getAllSdPaths(context);
        // 得到sd卡内image文件夹的路径   File.separator(/)
